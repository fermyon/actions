...
export async function updateComment(owner: string, repo: string, prNumber: number, msg: string): Promise<void> {
    if (!await findComment(owner, repo, prNumber, msg)) {
        await octokit.rest.issues.createComment({ owner: owner, repo: repo, issue_number: prNumber, body: msg })
        core.info(`updated comment on PR #${prNumber}`)
    }
}

export async function findComment(owner: string, repo: string, prNumber: number, msg: string): Promise<boolean> {
    for await (const response of octokit.paginate.iterator(
        octokit.rest.issues.listComments,
        { owner: owner, repo: repo, issue_number: prNumber, per_page: 100 }
    )) {
